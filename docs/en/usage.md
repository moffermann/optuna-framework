...
meta:
  ...
  optimization_adapter: myproj.optuna.optimization:MyOptimizationAdapter
...
## 3) TrialAdapter, WorkerAdapter, and OptimizationAdapter (execution hooks)
...
- `on_worker_start(context)` runs once when the worker starts.
- `on_worker_end(context)` runs once when the worker ends.
...
Example (OptimizationAdapter):

```python
from optuna_framework.adapters.optimization import OptimizationAdapter

class MyOptimizationAdapter(OptimizationAdapter):
...
- `--trial-adapter`, `--worker-adapter`, and `--optimization-adapter` for extra hooks.
